Shader "Custom/FogDisplayVision"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            { v2f o; o.vertex = UnityObjectToClipPos(v.vertex); o.worldPos = mul(unity_ObjectToWorld, v.vertex).xy; return o; }

            float circleSoft(float2 p, float2 c, float r, float f)
            { float d = distance(p, c); return saturate(1.0 - smoothstep(r - max(1e-6f,f), r + max(1e-6f,f), d)); }
